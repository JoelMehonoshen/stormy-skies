...
namespace Screwtape
{
    ...
    {
        public static float Round(float a_number, float a_roundFactor)
        {
            var multiplier = 1 / a_roundFactor;
            var large = Mathf.Round(multiplier * a_number);
            return large / multiplier;
        }

        public static float BezierInterp(float t)
        {
            return (t * t) * (3f - 2f * t);
        }
        #region Equations
